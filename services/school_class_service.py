...
def _validate(lop: LopInfo, is_update: bool = False) -> None:
    ...
    if not lop.khoi:
        raise ValueError("Khối không được để trống.")
    if not is_update and school_class_dao.get_by_id(lop.ma_lop):
        raise ValueError(f"Mã lớp '{lop.ma_lop}' đã tồn tại.")


def list_classes() -> List[LopInfo]:
    return school_class_dao.get_all()


def get_class(ma_lop: str) -> Optional[LopInfo]:
    return school_class_dao.get_by_id(ma_lop)
...
def student_count(ma_lop: str) -> int:
    return student_dao.count_by_class(ma_lop)


def generate_next_id() -> str:
    max_num = school_class_dao.get_max_ma_lop_num()
