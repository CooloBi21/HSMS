...
from ui.theme import BG_CARD, BORDER_LIGHT, TEXT_MUTED, apply_theme, toggle_theme, outline_button_kwargs
...
class HSMSApp(ctk.CTk):
    ...
    def _build_sidebar(self) -> None:
        sidebar = ctk.CTkFrame(self, width=220, corner_radius=0, fg_color=BG_CARD, border_width=1, border_color=BORDER_LIGHT)
        sidebar.grid(row=0, column=0, sticky="nsew")
        sidebar.grid_rowconfigure(7, weight=1)

        ctk.CTkLabel(
            sidebar, text="HSMS", font=ctk.CTkFont(size=24, weight="bold"), text_color=("#2563eb", "#60a5fa")
        ).grid(row=0, column=0, padx=20, pady=(24, 4), sticky="w")
        ctk.CTkLabel(
            sidebar, text="Quản lý Học sinh", font=ctk.CTkFont(size=12), text_color=("gray45", "gray60")
        ...
        self._theme_btn.grid(row=8, column=0, padx=14, pady=(8, 20), sticky="ew")
    ...
    def _build_content(self) -> None:
        ...
        self._views["dashboard"] = DashboardView(
            self._container, get_theme_mode=lambda: self._theme_mode, on_navigate=navigate
        )
        self._views["students"] = StudentsView(self._container, on_data_changed=on_change, on_navigate=navigate)
        self._views["classes"] = ClassesView(self._container, on_data_changed=on_change, on_navigate=navigate)
        self._views["teachers"] = TeachersView(self._container, on_data_changed=on_change, on_navigate=navigate)
